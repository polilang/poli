...
class BundleGet(Statement):
    """ BUNDLE.GET identifier, expression, <identifier> """
    ...
    def execute(self):
        if type(Interpreter.Objects.get(self.name)) == dict:
            Interpreter.Objects.put(
                self.target,
                Interpreter.Objects.get(self.name, key = self.key.evaluate())
            )
        else:
...
keywords.append('bundle.get')
nodes.update({'bundle.get': BundleGet})